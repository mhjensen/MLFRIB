...
%%%
\item[Description:] We have implemented several  ML methods for event classification in the Active-Target Time Projection Chamber detector at NSCL. In particular we have implemented advanced convolutional AE NN to the analysis of two-dimensional projections of particle tracks
from a resonant proton scattering experiment on $^{46}$Ar.


%%%
\item[FRIB relevance:] We expect that many of the new FRIB experiments will produce large amounts of data. To classify various events of interest is then a task which requires a large degree of automation. We expect that these techniques will play a central role at FRIB. 
%%%
\item[References:] {\it Machine learning methods for track classification in the AT-TPC}, \href{https://doi.org/10.1016/j.nima.2019.05.097}{Nucl. Inst. Meth. A940 (2019) 56-167}
\end{description}
%%%
\end{minipage}
}
...
%%%
\item[References:] {\it Bayesian approach to model-based extrapolation of nuclear observables}, \href{https://journals.aps.org/prc/abstract/10.1103/PhysRevC.98.034318}{Phys. Rev. C 98, 034318 (2018)} (Editor's Suggestion).
\end{description}
%%%
\end{minipage}
}
\begin{figure}[htb!]
\centering
\includegraphics[width=0.65\linewidth]{figures/Sn-sep.pdf}
\caption{
...
%%%
\item[Description:] ML methods offer several possibilities to
...
Renormalization Group set of equations and Coupled Cluster theory using DL algorithms, with a great deal of success using RNNs and LSTM based approaches, as well as KR based methods. Preliminary studies of infinite nuclear matters holds great promise for handling systems of large numbers of particles.
%%%
...
\caption{Extrapolations to large numbers of interacting fermions for a pairing model using KR methods on Coupled Cluster data.}
\end{figure}
\clearpage
\newpage
...
\begin{description}
%%%
\item[Participants:] S. König (TU Darmstadt/NCSU), A. Ekström (Chalmers), K. Hebeler (TU Darmstadt), D. Lee, and A. Schwenk (TU Darmstadt) 
%%%
\item[Description:] In applications of machine learning and uncertainty quantification to first principles nuclear many-body theory, one important tool is a fast and accurate emulator that removes the need for repeatedly performing computationally-expensive calculations.  We show that eigenvector continuation can fill this role quite nicely.
%%%
...
%%%
\item[References:] {\it Eigenvector Continuation as an Efficient and Accurate Emulator for Uncertainty Quantication in Nuclear Systems},  \href{https://arxiv.org/abs/1909.08446}{arXiv:1909.08446, submitted}.
\end{description}
%%%
\end{minipage}
}
...
\begin{description}
%%%
\item[Participants:] J. Bonitati (g), G. Given (g), Da. Lee (Cornell Tech/Samsung Research), and De. Lee
%%%
...
%%%
\item[References:] {\it Machine Learning Multi-Nucleon Correlations from First Principles}, in preparation.
\end{description}
%%%
\end{minipage}
}
